...
import { Archive, ArchiveRestore, Trash2, Search, X, FileText, StickyNote, FileEdit } from 'lucide-react';
...
export function ArchivePanel({
  ...
  const [confirmUnarchiveAll, setConfirmUnarchiveAll] = useState(false);
  ...
  }, [archive, search, filterType]);

  const handleUnarchiveAll = () => {
    archive.forEach(p => onUnarchivePage(p.id));
    setConfirmUnarchiveAll(false);
  };

  return (
    <div
      id="archive-panel-root"
      style={{
        display: 'flex',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <Archive size={15} style={{ color: c.accent }} />
            ...
            <div>
              {confirmUnarchiveAll ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <button
                    id="btn-cancel-unarchive-all"
                    onClick={() => setConfirmUnarchiveAll(false)}
                    style={{
                      fontSize: '0.68rem',
                      color: c.textMuted,
                ...
                <button
                  id="btn-unarchive-all-trigger"
                  onClick={() => setConfirmUnarchiveAll(true)}
                  style={{
                    fontSize: '0.7rem',
                    color: c.accent,
                    background: 'transparent',
                  ...
                  title={i18n.unarchiveAll}
                >
                  <ArchiveRestore size={12} />
                  <span>{i18n.unarchiveAll}</span>
                </button>
              )}
            </div>
          )}
        ...
        >
          ...
          <input
            ...
            placeholder={i18n.searchArchive}
            style={{
              background: 'transparent',
              border: 'none',
              outline: 'none',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, flex: 1 }}>
                    <TypeIcon size={14} style={{ color: c.accent, flexShrink: 0 }} />
                    <span
                      style={{
                        fontSize: '0.78rem',
                        fontWeight: 600,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    <button
                      id={`btn-unarchive-${p.id}`}
                      onClick={() => onUnarchivePage(p.id)}
                      title={i18n.unarchiveDocumentTooltip}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 3,
                    ...
                    >
                      <ArchiveRestore size={12} />
                      <span>{i18n.unarchive}</span>
                    </button>

                    <button
                      id={`btn-delete-${p.id}`}
                      onClick={() => onDeletePage(p.id)}
                      title={i18n.moveToTrashTooltip}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        padding: '3px 5px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.65rem', color: c.textMuted, paddingTop: 2 }}>
                  ...
                  {p.archivedAt && (
                    <span>
                      {new Date(p.archivedAt).toLocaleDateString(i18n.dateLocale, {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </span>
